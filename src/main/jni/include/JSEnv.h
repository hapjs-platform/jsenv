...
typedef bool (*UserFunctionCallback)(JSEnv*,
                                     void* user_data,
...
typedef bool (*JSFunctionCallback)(JSEnv* env,
                                   void* user_data,
                                   JSObject self,
                                   const JSValue* argv,
                                   int argc,
...
typedef bool (*JSPropertyGetCallback)(JSEnv* env,
                                      void* user_data,
                                      JSObject self,
                                      JSValue* pvalue);

typedef bool (*JSPropertySetCallback)(JSEnv* env,
                                      void* user_data,
                                      JSObject self,
...
class JSInspectorClient {
 public:
  ...
  virtual void SendResponse(int call_id,
                            const jschar_t* message,
                            size_t size) = 0;
...
class JSEnv {
 public:
  ...
  virtual bool GetGlobal(const JSValue* pkey,
                         JSValue* pvale,
                         uint32_t flags = 0) = 0;

  // object property
  virtual bool GetObjectProperty(JSObject object,
                                 const JSValue* pkey,
                                 JSValue* pvalue,
                                 uint32_t flags = 0) = 0;
  virtual bool SetObjectProperty(JSObject object,
                                 const JSValue* pkey,
                                 const JSValue* pvalue) = 0;
  template <typename TKey>
  bool GetObjectPropertyValue(JSObject object,
                              const TKey& key,
                              JSValue* pval,
                              uint32_t flags = 0) {
    JSValue jskey(key);
  ...
  template <typename TKey, typename TValue>
  bool SetObjectPropertyValue(JSObject object,
                              const TKey& key,
                              const TValue& value) {
    JSValue jskey(key);
    JSValue jsvalue(value);
  ...
  virtual bool GetObjectAtIndex(JSObject object,
                                int index,
                                JSValue* pvalue,
                                uint32_t flags = 0) = 0;
  virtual bool SetObjectAtIndex(JSObject object,
                                int index,
                                const JSValue* pvalue) = 0;
  ...
  // function call
  virtual JSObject NewFunction(JSFunctionCallback callback,
                               void* user_data,
                               uint32_t flags = 0) = 0;

  virtual bool CallFunction(JSObject function,
                            JSObject self,
                            const JSValue* argv,
                            int argc,
                            JSValue* presult,
  ...
  virtual bool GetPromiseResult(JSObject promise,
                                JSValue* pvalue,
                                uint32_t flags = 0) = 0;
